 ...
 *
 */
public class MattermostFilterTranslator extends AbstractFilterTranslator<MattermostFilter> {
    private static final Log LOG = Log.getLog(MattermostFilterTranslator.class);

    @Override
    protected MattermostFilter createEqualsExpression(EqualsFilter filter, boolean not) {
        ...
        if (Uid.NAME.equals(attr.getName())) {
            if (attr.getValue() != null && attr.getValue().get(0) != null) {
            	MattermostFilter lf = new MattermostFilter();
